...
export default function HomePage() {
  ...
  useEffect(() => {
    // Load products from the database via /api/products. The Pi SDK is never
    // touched here, so a normal browser can never auto-enter a "Connecting…"
    // state — that only happens after the user taps Login with Pi.
    let active = true;
    fetchProducts()
      .then((list) => {
    ...
    <>
      <main className="flex flex-1 flex-col px-5 pt-safe">
        ...
        <section className="pt-10">
          ...
          {/* Pi login — full state machine + debug panel live in PiLogin. */}
          <div className="mt-6">
            <PiLogin />
          </div>
        </section>
        ...
        <section className="mt-9 flex-1">
          ...
          {!ready ? (
            ...
            <p className="rounded-card bg-danger/10 px-4 py-4 text-center text-[14px] leading-relaxed text-danger">
              {error}
            </p>
          ) : products.length === 0 ? (
