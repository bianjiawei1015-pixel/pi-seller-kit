...
import { getPiAccessToken, isPiBrowser } from "@/lib/pi";
import { createProductApi } from "@/lib/client";
...
export default function CreateProductPage() {
  ...
  async function handleSubmit() {
    ...
    // Products are owned by the seller's Pi account. Require a login first (for a
    // clear prompt) — but the real ownership check happens on the server, which
    // derives seller_uid from a freshly verified Pi access token below.
    const seller = getPiUser();
    if (!seller) {
      return setError(
        "Please log in with Pi on the home page before creating a product.",
      );
    }
    if (!isPiBrowser()) {
      return setError(
        "Please open this app in Pi Browser to create a product.",
      );
    }

    setSaving(true);
    try {
      // Obtain a fresh Pi access token and send it with the request. The server
      // verifies it and sets seller_uid from the verified identity — the client
      // never sends (and the server never trusts) a raw seller uid. The token is
