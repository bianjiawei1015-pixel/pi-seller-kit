...
function extractAccessToken(raw: unknown): string {
  const maybe = raw as { accessToken?: unknown };
  return typeof maybe?.accessToken === "string" ? maybe.accessToken : "";
}

// Runs Pi.authenticate and returns a FRESH access token. The token lives only in
// memory for the duration of the call that uses it; it is never written to
// localStorage. Both loginWithPi (which verifies it on the server) and
// getPiAccessToken (for privileged writes like creating a product) build on this.
async function authenticateForAccessToken(): Promise<string> {
  ...
  if (!accessToken) throw new Error("Pi login returned no access token.");
  return accessToken;
}

// Exchanges an access token for a SERVER-VERIFIED identity. The backend calls
// the Pi Platform /me endpoint, upserts the user, and returns only the public
// uid + username. The token is sent once and never stored on the client.
async function verifyWithServer(accessToken: string): Promise<PiLoginUser> {
  ...
  if (
    ...
    throw new Error(data?.message || "Pi verification failed on the server.");
  }
  return { uid, username };
}

// Logs the user in: authenticate with the Pi SDK, then verify the access token
// on the server. Returns the SERVER-VERIFIED uid + username (the caller stores
// only those). The access token is never returned or persisted.
export async function loginWithPi(): Promise<PiLoginUser> {
  const run = (async (): Promise<PiLoginUser> => {
    const accessToken = await authenticateForAccessToken();
    return verifyWithServer(accessToken);
  })();
...
// Returns a fresh Pi access token for a SINGLE privileged server request (for
// example, creating a product where the server must derive seller_uid from a
// verified identity). The token is handed to the caller, included in that one
// request body, and never stored in localStorage.
export async function getPiAccessToken(): Promise<string> {
  return authenticateForAccessToken();
}
