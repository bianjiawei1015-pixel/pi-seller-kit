...
export async function POST(req: Request) {
  let body: { paymentId?: string; txid?: string; orderId?: string };
  try {
    body = await req.json();
  } catch {
  ...
  const orderId = body.orderId || paymentId;
  if (!paymentId) {
    return NextResponse.json(
  ...
  if (!txid) {
    return NextResponse.json(
      { ok: false, message: "txid is required" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        {
          ok: false,
          message: `Pi complete failed (${res.status})`,
          details: payment,
        },
        { status: res.status },
      );
    ...
    return NextResponse.json({ ok: true, paymentId, txid, orderId, payment });
  } catch (err) {
    return NextResponse.json(
      {
        ok: false,
